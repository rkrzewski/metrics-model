...
object LineProtocol {
  ...
  def writeMetric[M <: Metric, R <: HList, S <: HList](m: M)(
    implicit lgen: LabelledGeneric.Aux[M, R],
    mapper: Mapper.Aux[writeField.type, R, S],
  ...
  object writeMetricsMapField extends Poly1 {
    implicit def default[K <: Symbol, M <: Map[String, Metric], R <: HList, S <: HList](
      implicit kw: Witness.Aux[K],
      lgen: LabelledGeneric.Aux[Metric, R],
      mapper: Mapper.Aux[writeField.type, R, S],
      totr: ToTraversable.Aux[S, List, String]): Case.Aux[FieldType[K, M], Seq[(String, String)]] =
      at[FieldType[K, M]](m => m.map {
        case (l, v) => (s"${kw.value.name}.${l}", writeMetric(v))
      }.toSeq)
  }

  def writeMetrics[R <: HList, S <: HList](m: Metrics, tags: String, timestamp: String)(
    implicit lgen: LabelledGeneric.Aux[Metrics, R],
    mapper: Mapper.Aux[writeMetricsMapField.type, R, S],
    totr: ToTraversable.Aux[S, List, (String, String)]): String = {
